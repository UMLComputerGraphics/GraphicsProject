...
int findTriangleWithMinimumDistance(Object* largerModel, Object* smallerModel, int index){
	...
	for(int i=0; i<smallerModel->getNumberPoints();i+= 3){
		float distance;
		for(int j=0; j<3; j++){
			distance += threeDimensionalDistance(smallerModel->points[i+j],largerModel->points[index+j]);
		}
...
void makeModelsSameSize(Object* model1, Object* model2){
	if(model1->getNumberPoints()>model2->getNumberPoints()){
		while(model2->getNumberPoints() < model1->getNumberPoints()){
			int index = findTriangleWithMinimumDistance(model1,model2,model2->getNumberPoints());
			for(int i=0; i<3; i++){
		...
		while(model1->getNumberPoints() < model2->getNumberPoints()){
			int index = findTriangleWithMinimumDistance(model2,model1,model1->getNumberPoints());
			for(int i=0; i<3; i++){
