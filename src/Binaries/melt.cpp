    ...
    for ( i = 0; i <= (slices / 2); i++ ) {
      glVertex3f(
          ...
          * cos( (float) 2 * i / slices * M_PI ),
          -radius * sin( (float) (j + 1) / slices * M_PI / 2.0 ) * grow
          + positiony[i],
          radius * cos( (float) (j + 1) / slices * M_PI / 2.0 )
      ...
      glVertex3f(
          ...
          * cos( (float) 2 * i / slices * M_PI ),
          -radius * sin( (float) j / slices * M_PI / 2.0 ) * grow
          + positiony[i],
          radius * cos( (float) j / slices * M_PI / 2.0 )
          * sin( (float) 2 * i / slices * M_PI ) );
    }
...
void camera( void ) {
   ...
   */
  gluLookAt( -25, candleheight + 25, -75, 0, candleheight, 0, -25,
             candleheight + 26, -75 );
}
...
void _keyboard( unsigned char key, int x, int y ) {
  ...
  if ( key == ' ' ) {
    ...
    if ( grow >= 0.5 ) {
      ...
      if ( candleheight >= 5 ) candleheight -= random;
    }
  }
}
