...
void draw (void)
{
  ...
  for(j = 0; j <= slices; j++)
	...
	{
	  glVertex3f(  
 radius*cos((float)(j + 1) / slices*M_PI / 2.0 )*cos((float)2*i / slices*M_PI),
-radius*sin((float)(j + 1) / slices*M_PI / 2.0 )*grow +  positiony[i],
 radius*cos((float)(j + 1) / slices*M_PI / 2.0 )*sin((float)2*i / slices*M_PI)
		    );
	  
	  glVertex3f(
 radius*cos((float)j / slices*M_PI / 2.0 )*cos((float)2*i / slices*M_PI),
...
int main (int argc, char **argv)
{
  ...
  GLEW_INIT(); // set OpenGL state and initialize shaders
