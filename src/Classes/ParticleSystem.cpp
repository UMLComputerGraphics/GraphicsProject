...
vec4 ParticleSystem::getRandomCircularSpawnPoint( void ) {
  ...
  jitteredRadius = rangeRandom( this->_emitterRadius / 100,
                                this->_emitterRadius );
...
vec4 ParticleSystem::getRandomHemisphericalSpawnPoint( void ) {
...
// don't use this
void ParticleSystem::addOneParticleAtOrigin( void ) {
  
  Particle *p = new Particle( vec4( 0.0, 0.0, 0.0, 1.0 ), 1,
                              generateLifespan() );
...
void ParticleSystem::setSlaughterHeight( float f ) {
  this->_slaughterHeight = f;
}

void ParticleSystem::addSomeParticles( int numToAdd ) {
...
void ParticleSystem::fillSystemWithParticles( void ) {
  
  int numParticles = getNumParticles();
  
  while ( numParticles % 3 )
    numParticles++;
...
bool ParticleSystem::getParticleSpace( void ) const {
  return _useGlobalParticleSpace;
}

void ParticleSystem::setParticleSpace( bool flag ) {
  _useGlobalParticleSpace = flag;
}

void ParticleSystem::setEmitterRadius( float r ) {
...
void ParticleSystem::update() {
  ...
  for ( i = _particles.begin(); i != _particles.end(); ++i ) {
    ...
    (*i)->updateSelf();
    (*i)->setColor(
        (*_colorFunc)( percentLifeRemaining, (*i)->getPosition() ) );
...
void ParticleSystem::setColorFunc( vec4 (*ColorFunc)( float, vec4 ) ) {
...
float ParticleSystem::generateLifespan() {
  ...
  float maxLifeMinor = ((this->_maxLife - this->_minLife) * .3)
      + this->_minLife;
  int dieRoll = rand();
  float life = 0.0;
