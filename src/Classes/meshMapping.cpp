...
void populateSrcSquare( SquareMap* squareMap,
                        std::vector< Angel::vec4 > points ) {
  for ( int i = 0; i < int( points.size() ); i += 3 ) {
    if ( isInside( 0.0, 0.0, squareMap->xStart, squareMap->zStart,
                   squareMap->xEnd, squareMap->zStart, float( points[i].x ),
                   float( points[i].z ) ) ) {
      //front
      for ( int j = 0; j < 3; j++ ) {
        squareMap->front->srcTriangles.push_back( points[i + j] );
      }
    } else if ( isInside( 0.0, 0.0, squareMap->xStart, squareMap->zEnd,
                          squareMap->xEnd, squareMap->zEnd,
                          float( points[i].x ), float( points[i].z ) ) ) {
      //back
      for ( int j = 0; j < 3; j++ ) {
        squareMap->back->srcTriangles.push_back( points[i + j] );
      }
    } else if ( isInside( 0.0, 0.0, squareMap->xStart, squareMap->zStart,
                          squareMap->xStart, squareMap->zEnd,
                          float( points[i].x ), float( points[i].z ) ) ) {
      //left
      for ( int j = 0; j < 3; j++ ) {
...
void populateDestSquare( SquareMap* squareMap,
                         std::vector< Angel::vec4 > points ) {
...
float triangleArea( float x1, float y1, float x2, float y2, float x3,
                    float y3 ) {
  return std::abs( (x1 * (y2 - y3) + x2 * (y3 - y1) + x3 * (y1 - y2)) / 2.0 );
}

bool isInside( float x1, float y1, float x2, float y2, float x3, float y3,
               float x, float y ) {
...
void expandSquareMap( SquareMap* squareMap ) {
  expandMap( squareMap->front, squareMap->front->xStart, squareMap->front->xEnd,
             squareMap->front->yStart, squareMap->front->yEnd );
  expandMap( squareMap->back, squareMap->back->xStart, squareMap->back->xEnd,
             squareMap->back->yStart, squareMap->back->yEnd );
  expandMap( squareMap->left, squareMap->left->xStart, squareMap->left->xEnd,
             squareMap->left->yStart, squareMap->left->yEnd );
  expandMap( squareMap->right, squareMap->right->xStart, squareMap->right->xEnd,
             squareMap->right->yStart, squareMap->right->yEnd );
}

void expandMap( Square* square, float xStart, float xEnd, float yStart,
                float yEnd ) {
...
Square* subdivideBottomLeft( Square* square ) {
  return new Square( square->xStart,
                     square->xStart + (square->xEnd - square->xStart) / 2,
                     square->yStart,
                     square->yStart + (square->yEnd - square->yStart) / 2 );
}
...
Square* subdivideBottomRight( Square* square ) {
  return new Square( square->xStart + (square->xEnd - square->xStart) / 2,
                     square->xEnd, square->yStart,
                     square->yStart + (square->yEnd - square->yStart) / 2 );
}
