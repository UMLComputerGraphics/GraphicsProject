...
void BipartiteGraph::constructGraph() {
  ...
  for ( size_t i = 0; i < srcNodes.size(); i++ ) {
    for ( size_t j = 0; j < destNodes.size(); j++ ) {
      ...
      findOptimalOrientation( destNodes[j]->a, destNodes[j]->b, destNodes[j]->c,
                              srcNodes[i]->a, srcNodes[i]->b, srcNodes[i]->c,
                              point1, point2, point3 );
      float distance = threeDimensionalDistance( point1, srcNodes[i]->a )
          + threeDimensionalDistance( point2, srcNodes[i]->b )
          + threeDimensionalDistance( point3, srcNodes[i]->c );
...
void BipartiteGraph::createMatches() {
  ...
  for ( size_t i = 0; i < edges.size(); i++ ) {
    if ( (!edges[i]->source->matched) && (!edges[i]->destination->matched)
         && (!edges[i]->destination->hasBetterMatch) ) {
      ...
      bestEdges.push_back(
          new Edge( edges[i]->distance, edges[i]->source,
                    edges[i]->destination ) );
    ...
    } else if ( (edges[i]->source->matched)
        && (!edges[i]->destination->matched) ) {
      edges[i]->destination->hasBetterMatch = true;
    }
  }
  ...
  for ( size_t i = 0; i < bestEdges.size(); i++ ) {
    if ( bestEdges[i]->source->minDistanceIndex
        != bestEdges[i]->destination->nodeIndex ) {
      firstBirths.push_back(
          new Edge( bestEdges[i]->distance, bestEdges[i]->source,
                    bestEdges[i]->destination ) );
    }
  }
  ...
  for ( size_t i = 0; i < bestEdges.size(); i++ ) {
    if ( bestEdges[i]->source->minDistanceIndex
      ...
      for ( int m = 0; m < 3; m++ ) {
        ...
        src->_vertices[3 * i + m] = src->_vertices[bestEdges[i]->source
                                                       ->modelIndex
                                                   + m];
        src->_normals[3 * i + m] = src->_normals[bestEdges[i]->source
                                                     ->modelIndex
                                                 + m];
        src->_colors[3 * i + m] = src->_colors[bestEdges[i]->source->modelIndex
            + m];
        ...
        dest->_vertices[3 * i + m] = dest->_vertices[bestEdges[i]->destination
                                                         ->modelIndex
                                                     + m];
        dest->_normals[3 * i + m] = dest->_normals[bestEdges[i]->destination
                                                       ->modelIndex
                                                   + m];
        dest->_colors[3 * i + m] = dest->_colors[bestEdges[i]->destination
                                                     ->modelIndex
                                                 + m];
  ...
  for ( size_t i = bestEdges.size() - firstBirths.size(); i < bestEdges.size();
      i++ ) {
    for ( int m = 0; m < 3; m++ ) {
  ...
  for ( size_t i = 0; i < bestEdges.size(); i++ ) {
    if ( bestEdges[i]->source->minDistanceIndex
      ...
      for ( int m = 0; m < 3; m++ ) {
        ...
        srcVertices[3 * i + m] = srcVertices[bestEdges[i]->source->modelIndex
            + m];
        srcNormals[3 * i + m] =
            srcNormals[bestEdges[i]->source->modelIndex + m];
        ...
        destVertices[3 * i + m] = destVertices[bestEdges[i]->destination
                                                   ->modelIndex
                                               + m];
        destNormals[3 * i + m] = destNormals[bestEdges[i]->destination
                                                 ->modelIndex
                                             + m];
        destColors[3 * i + m] = destColors[bestEdges[i]->destination->modelIndex
            + m];
      ...
      printf( "Min Index: %d  Node Index: %d\n",
              bestEdges[i]->source->minDistanceIndex,
              bestEdges[i]->destination->nodeIndex );
      bestEdges.erase( bestEdges.begin() + i );
      for ( int m = 0; m < 3; m++ ) {
        srcVertices.erase( srcVertices.begin() + 3 * i );
