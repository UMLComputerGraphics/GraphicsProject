...
#ifdef _RT
  clock_gettime( CLOCK_REALTIME, &_T2 ) ;
  this->_delta = (_T2.tv_nsec - _T1.tv_nsec) + (SecToNSec * (_T2.tv_sec - _T1.tv_sec)) ;
  this->_scale = (this->_delta / (KeyFrameRate * 1000))
#else
  gettimeofday(&_T2, NULL) ;
  this->_delta = (_T2.tv_usec - _T1.tv_usec)
      + (SecTouSec * (_T2.tv_sec - _T1.tv_sec)) ;
  this->_scale = (this->_delta / KeyFrameRate) ;
#endif
  _T1 = _T2 ;
  return _delta ;
}
