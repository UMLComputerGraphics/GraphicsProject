...
void MLAlg::train( const CvMat *trainIn, const CvMat *trainOut ) {
  cout << "Training " << MLAlgName[tp] << " with " << trainIn->rows
  ...
  case Boost:
    boost.train( trainIn, CV_ROW_SAMPLE, trainOut, 0, 0, var_type, 0,
                 boostparams );
    break;
...
void MLAlg::validate( const CvMat *validateIn, const CvMat *validateOut ) {
  cout << "Validating " << MLAlgName[tp] << " with " << validateIn->rows
  << " training samples." << endl;
  ...
  for ( int v = 0; v < validateIn->rows; v++ ) {
    ...
    err += (cat != r);
    cout << "True value: " << categoryName( cat ) << " - " << MLAlgName[tp]
         << ": " << categoryName( r ) << "  " << (r == cat ? "" : "ERROR")
         << endl;
  }
  cout << "Errors = " << err << "/" << validateIn->rows << " - Accuracy = "
  << (1.0f - (float) err / validateIn->rows) * 100 << " %" << endl;
  cout << "Done." << endl;
}

void MLAlg::recognize( const CvMat *recognizeIn, CvMat *recognizeOut ) {
  cout << "Recognizing the acquired gesture with " << MLAlgName[tp] << "..."
       << endl;
