...
MLData::MLData( const vector< int >& mask ) :
    rng( time( 0 ) ) {
  all_in = all_out = NULL;
  ...
  if ( !mask.size() ) cout << "Feature Mask is empty!" << endl;
  featureMask = mask;
}
  ...
  for ( int i = 0; i < ncategories; i++ ) {
    ...
    while ( ff.good() ) {
      ...
      if ( nfeatures == 0 ) {
        ...
        ss >> val;
        while ( ss.good() ) {
          if ( find( featureMask.begin(), featureMask.end(), (k + 1) )
               != featureMask.end() ) { // Feature selection
            a[nfeatures++] = val;
          }
          ss >> val;
          k++;
...
bool MLData::save( string savefile, bool add_flag ) {
  ...
  if ( add_flag ) out.open( savefile.c_str(), ios::app );
  else out.open( savefile.c_str(), ios::trunc );
...
bool MLData::loadTraining( const Training* t, float timeDelta ) {
  ...
  for ( int k = 0; k < nfeatures; k++ ) {
    ...
    if ( features[k] < min ) min = features[k];
    else if ( features[k] > max ) max = features[k];
  }
}

void MLData::computeDisplacement( const Training* t,
                                  vector< double >& features ) {
  ...
  for ( int i = 0; i < t->size(); i++ ) {
    const AccSample* sample = reinterpret_cast< const AccSample* >( t->sampleAt(
        i ) );
    xspace = xspace
             + xvel
               * timeDelta_+ 0.5*sample->x()*timeDelta_*timeDelta_*GRAV_ACC;xvel=
    xvel + sample->x() * timeDelta_ * GRAV_ACC;
    yspace = yspace
             + yvel
               * timeDelta_+ 0.5*sample->y()*timeDelta_*timeDelta_*GRAV_ACC;yvel=
    yvel + sample->y() * timeDelta_ * GRAV_ACC;
    zspace =
        zspace
        + zvel * timeDelta_+ 0.5*(sample->z()-1)*timeDelta_*timeDelta_*GRAV_ACC;zvel=
    zvel + (sample->z() - 1) * timeDelta_ * GRAV_ACC;
  }
...
void MLData::generateTrainingAndValidationData( float perc )// genera random perc % training data (in train) e il resto come validation
  ...
  if ( fabs( perc - 1.0f ) < 1e-9 ) {
    ...
    cout << "Training samples = " << ts << ", Validation samples = " << vs
         << endl;
    ...
    vector< bool > forvalidation;
    forvalidation.reserve( nsamples );
    for ( int k = 0; k < nsamples; k++ )
      forvalidation[k] = false;
    ...
    while ( cvs < vs ) {
      ...
      if ( !forvalidation[r] ) {
        ...
        for ( int k = 0; k < nfeatures; k++ ) {
          validation_in->data.fl[cvs * nfeatures + k] =
              all_in->data.fl[r * nfeatures + k];
        }
    ...
    for ( int r = 0; r < nsamples; r++ ) {
      if ( !forvalidation[r] ) {
        for ( int k = 0; k < nfeatures; k++ ) {
          train_in->data.fl[cts * nfeatures + k] = all_in->data.fl[r * nfeatures
                                                                   + k];
        }
...
string MLData::categoryName( int i ) // name of k-th category
    {
  if ( i < categoryNames.size() ) return categoryNames[i];
  else {
    ostringstream oss;
    oss << i;
    return oss.str();
