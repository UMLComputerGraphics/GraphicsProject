 ...
 */
CButtonBase::CButtonBase( void *ButtonsPtr, void *ButtonsHeldPtr,
                          void *ButtonsReleasedPtr ) {
...
int CButtonBase::isJustPressed( int Button ) {
  return ((Cast( mpBtnsPtr ) & Button) == Button)
         && ((Cast( mpBtnsHeldPtr ) & Button) != Button);
}

/*
 * Initializers for classes derrived from CButtonBase.
 */
CButtons::CButtons( void *ButtonsPtr, void *ButtonsHeldPtr,
                    void *ButtonsReleasedPtr ) :
    CButtonBase( ButtonsPtr, ButtonsHeldPtr, ButtonsReleasedPtr ) {
}

CNunchukButtons::CNunchukButtons( void *ButtonsPtr, void *ButtonsHeldPtr,
                                  void *ButtonsReleasedPtr ) :
    CButtonBase( ButtonsPtr, ButtonsHeldPtr, ButtonsReleasedPtr ) {
}

CClassicButtons::CClassicButtons( void *ButtonsPtr, void *ButtonsHeldPtr,
                                  void *ButtonsReleasedPtr ) :
    CButtonBase( ButtonsPtr, ButtonsHeldPtr, ButtonsReleasedPtr ) {
}

CGH3Buttons::CGH3Buttons( void *ButtonsPtr, void *ButtonsHeldPtr,
                          void *ButtonsReleasedPtr ) :
    CButtonBase( ButtonsPtr, ButtonsHeldPtr, ButtonsReleasedPtr ) {
}
...
CAccelerometer::CAccelerometer( struct accel_t *AccelCalPtr,
                                struct vec3b_t *AccelerationPtr,
                                int *AccelThresholdPtr,
                                struct orient_t *OrientationPtr,
                                float *OrientationThresholdPtr,
 ...
 */
CGyroscope::CGyroscope( struct ang3s_t* RawGyro, struct ang3s_t* CalGyro,
                        struct ang3f_t* AngleRate, unsigned char* Mode,
                        struct motion_plus_t* MPPtr, int* GyroThresholdPtr ) {
  mpRawGyro = RawGyro;
  mpCalGyro = CalGyro;
 ...
 */
CWeightSensor::CWeightSensor( struct pressure_t* RawWeight,
                              struct pressure_t* LowCalWeight,
                              struct pressure_t* MediumCalWeight,
                              struct pressure_t* HighCalWeight,
                              struct pressure_weight_t* Weight,
                              struct balance_board_t* BBPtr ) {
...
void CWeightSensor::GetRawWeight( int& TopLeft, int& TopRight, int& BottomLeft,
                                  int& BottomRight ) {
...
void CWeightSensor::GetLowCalWeight( int& TopLeft, int& TopRight,
...
void CWeightSensor::GetMediumCalWeight( int& TopLeft, int& TopRight,
                                        int& BottomLeft, int& BottomRight ) {
...
void CWeightSensor::GetHighCalWeight( int& TopLeft, int& TopRight,
                                      int& BottomLeft, int& BottomRight ) {
...
void CWeightSensor::GetWeight( float& TotalWeight, float& TopLeft,
...
std::vector< CIRDot >& CIR::GetDots() {
  ...
  for ( index = 0; index < 4; index++ ) {
    ...
    if ( dot.isVisible() ) mpIRDotsVector.push_back( dot );
  }
...
CExpansionDevice::CExpansionDevice( struct expansion_t * ExpPtr ) :
    ...
    MotionPlus( ExpPtr ), BalanceBoard( ExpPtr ) {
...
CNunchuk::CNunchuk( struct expansion_t *ExpPtr ) :
    Buttons( (void *) &(ExpPtr->nunchuk.btns),
             (void *) &(ExpPtr->nunchuk.btns_held),
             (void *) &(ExpPtr->nunchuk.btns_released) ),
    Joystick( &(ExpPtr->nunchuk.js) ),
    Accelerometer( &(ExpPtr->nunchuk.accel_calib), &(ExpPtr->nunchuk.accel),
                   &(ExpPtr->nunchuk.accel_threshold),
                   &(ExpPtr->nunchuk.orient),
                   &(ExpPtr->nunchuk.orient_threshold),
                   &(ExpPtr->nunchuk.gforce) ) {
}
...
CClassic::CClassic( struct expansion_t *ExpPtr ) :
    Buttons( (void *) &(ExpPtr->classic.btns),
             (void *) &(ExpPtr->classic.btns_held),
             (void *) &(ExpPtr->classic.btns_released) ),
    LeftJoystick( &(ExpPtr->classic.ljs) ),
    RightJoystick( &(ExpPtr->classic.rjs) ) {
...
CGuitarHero3::CGuitarHero3( struct expansion_t *ExpPtr ) :
    Buttons( (void *) &(ExpPtr->gh3.btns), (void *) &(ExpPtr->gh3.btns_held),
             (void *) &(ExpPtr->gh3.btns_released) ),
...
CMotionPlus::CMotionPlus( struct expansion_t *ExpPtr ) :
    Gyroscope( &(ExpPtr->mp.raw_gyro), &(ExpPtr->mp.cal_gyro),
               &(ExpPtr->mp.gyro_rate), (unsigned char*) &(ExpPtr->mp.acc_mode),
               &(ExpPtr->mp), &(ExpPtr->mp.raw_gyro_threshold) ) {
  mpMPPtr = &(ExpPtr->mp);
}
...
CBalanceBoard::CBalanceBoard( struct expansion_t *ExpPtr ) :
    WeightSensor( &(ExpPtr->bb.pressure_raw_data), &(ExpPtr->bb.cal_low_weight),
                  &(ExpPtr->bb.cal_medium_weight),
                  &(ExpPtr->bb.cal_high_weight), &(ExpPtr->bb.pressure_weight),
                  &(ExpPtr->bb) ) {
  // Initialize the expansion pointer.
  mpBBPtr = &(ExpPtr->bb);
}
...
CWiimote::CWiimote() :// SWIG insisted it exist for the vectors. Hopefully it will only be used for copy.
    ...
    Accelerometer( (accel_t*) NULL, (vec3b_t*) NULL, (int*) &(mpTempInt),
                   (orient_t*) NULL, (float*) &(mpTempFloat),
                   (gforce_t*) NULL ),
...
CWiimote::CWiimote( struct wiimote_t *wmPtr ) :
    IR( wmPtr ),
    Buttons( (void *) &(wmPtr->btns), (void *) &(wmPtr->btns_held),
             (void *) &(wmPtr->btns_released) ),
    Accelerometer( (accel_t*) &(wmPtr->accel_calib), (vec3b_t*) &(wmPtr->accel),
                   (int*) &(wmPtr->accel_threshold),
                   (orient_t*) &(wmPtr->orient),
                   (float*) &(wmPtr->orient_threshold),
                   (gforce_t*) &(wmPtr->gforce) ),
...
CWiimote::CWiimote( const CWiimote &copyin ) :// Copy constructor to handle pass by value.
    ...
    Buttons( (void *) &(copyin.mpWiimotePtr->btns),
             (void *) &(copyin.mpWiimotePtr->btns_held),
             (void *) &(copyin.mpWiimotePtr->btns_released) ),
    Accelerometer( (accel_t*) &(copyin.mpWiimotePtr->accel_calib),
                   (vec3b_t*) &(copyin.mpWiimotePtr->accel),
                   (int*) &(copyin.mpWiimotePtr->accel_threshold),
                   (orient_t*) &(copyin.mpWiimotePtr->orient),
                   (float*) &(copyin.mpWiimotePtr->orient_threshold),
                   (gforce_t*) &(copyin.mpWiimotePtr->gforce) ),
...
void CWiimote::EnableMotionPlus( CWiimote::OnOffSelection State ) {
  if ( State == CWiimote::ON ) ExpansionDevice.MotionPlus.Connect(
      mpWiimotePtr );
  else ExpansionDevice.MotionPlus.Disconnect( mpWiimotePtr );
}
...
void CWiimote::ReadData( unsigned char *Buffer, unsigned int Offset,
                         unsigned int Length ) {
  wiic_read_data( mpWiimotePtr, Buffer, Offset, Length );
}

void CWiimote::WriteData( unsigned int Address, unsigned char *Data,
                          unsigned int Length ) {
...
void CWiimote::SetSmoothing( bool Smooth ) {
  if ( Smooth ) wiic_set_flags( mpWiimotePtr, WIIC_SMOOTHING, 0 );
  else wiic_set_flags( mpWiimotePtr, 0, WIIC_SMOOTHING );
}
...
int CWii::Find( int Timeout ) {
  return wiic_find( (struct wiimote_t**) mpWiimoteArray, mpWiimoteArraySize,
                    Timeout );
}
...
std::vector< CWiimote >& CWii::Connect( bool autoreconnect ) {
  ...
  numConnected = wiic_connect( (struct wiimote_t**) mpWiimoteArray,
                               mpWiimoteArraySize, autoreconnect );
...
std::vector< CWiimote >& CWii::FindAndConnect( int timeout, bool rumbleAck,
  ...
  cout << "Connected to " << (unsigned int) mpWiimotesVector.size()
       << " wiimotes" << endl;
  
  // Setup the wiimotes
  for ( index = 0, i = mpWiimotesVector.begin(); i != mpWiimotesVector.end();
      ++i, ++index ) {
...
int CWii::Poll() {
  int events = wiic_poll( (struct wiimote_t**) mpWiimoteArray,
                          mpWiimoteArraySize );
  
  // Logging
  for ( std::vector< CWiimote >::iterator i = mpWiimotesVector.begin();
      i != mpWiimotesVector.end(); ++i )
    if ( (*i).isLogEnabled() ) (*i).Log();
