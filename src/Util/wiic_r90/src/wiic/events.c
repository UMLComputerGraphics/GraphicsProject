...
int wiic_poll( struct wiimote_t** wm, int wiimotes ) {
  ...
  for ( i = 0; i < wiimotes; ++i ) {
    ...
    if ( WIIMOTE_IS_SET(wm[i], WIIMOTE_STATE_CONNECTED) ) {
      ...
      if ( wm[i]->in_sock > highest_fd ) highest_fd = wm[i]->in_sock;
    }
    ...
    if ( FD_ISSET(wm[i]->in_sock, &fds) ) {
      ...
      if ( r == -1 ) {
        ...
        if ( errno == ENOTCONN ) {
          ...
          WIIC_ERROR(
              "Bluetooth appears to be disconnected.  Wiimote unid %i will be disconnected.",
              wm[i]->unid );
...
void propagate_event( struct wiimote_t* wm, byte event, byte* msg ) {
  ...
  case WM_RPT_BTN_ACC: {
    ...
    calculate_gforce( &wm->accel_calib, &wm->accel, &wm->gforce,
                      WIIMOTE_IS_FLAG_SET(wm, WIIC_SMOOTHING) );
  ...
  /* was there an event? */
  if ( state_changed( wm ) ) wm->event = WIIC_EVENT;
}
...
static void event_data_read( struct wiimote_t* wm, byte* msg ) {
  ...
  if ( err ) {
    ...
    if ( wm->read_req ) wiic_send_next_pending_read_request( wm );
...
static void event_status( struct wiimote_t* wm, byte* msg ) {
  ...
  if ( (msg[2] & WM_CTRL_STATUS_BYTE1_ATTACHMENT)
       == WM_CTRL_STATUS_BYTE1_ATTACHMENT ) attachment = 1;
  ...
  if ( (msg[2] & WM_CTRL_STATUS_BYTE1_IR_ENABLED)
       == WM_CTRL_STATUS_BYTE1_IR_ENABLED ) ir = 1;
...
void handshake_expansion( struct wiimote_t* wm, byte* data, unsigned short len ) {
  ...
  if ( !data ) {
    if ( WIIMOTE_IS_SET(wm, WIIMOTE_STATE_EXP)
         || WIIMOTE_IS_SET(wm, WIIMOTE_STATE_EXP_FAILED)
         || WIIMOTE_IS_SET(wm, WIIMOTE_STATE_EXP_HANDSHAKE) ) return;
    
    byte* handshake_buf;
    byte buf = 0x00;
    
    if ( WIIMOTE_IS_SET(wm, WIIMOTE_STATE_EXP) ) disable_expansion( wm );
  ...
  case EXP_ID_CODE_NUNCHUK: {
    if ( nunchuk_handshake( wm, &wm->exp.nunchuk, data, len ) ) wm->event =
        WIIC_NUNCHUK_INSERTED;
    break;
  }
  case EXP_ID_CODE_CLASSIC_CONTROLLER: {
    if ( classic_ctrl_handshake( wm, &wm->exp.classic, data, len ) ) wm->event =
        WIIC_CLASSIC_CTRL_INSERTED;
    break;
  }
  case EXP_ID_CODE_GUITAR: {
    if ( guitar_hero_3_handshake( wm, &wm->exp.gh3, data, len ) ) wm->event =
        WIIC_GUITAR_HERO_3_CTRL_INSERTED;
    break;
  }
  case EXP_ID_BALANCE_BOARD: {
    if ( balance_board_handshake( wm, &wm->exp.bb, data, len ) ) wm->event =
...
static void save_state( struct wiimote_t* wm ) {
  ...
  case EXP_BALANCE_BOARD:
    wm->lstate.pressure_raw_data.top_left = wm->exp.bb.pressure_raw_data
        .top_left;
    wm->lstate.pressure_raw_data.top_right = wm->exp.bb.pressure_raw_data
        .top_right;
    wm->lstate.pressure_raw_data.bottom_left = wm->exp.bb.pressure_raw_data
        .bottom_left;
    wm->lstate.pressure_raw_data.bottom_right = wm->exp.bb.pressure_raw_data
        .bottom_right;
    break;
...
#define CROSS_THRESH_RATE(last, now, thresh)								\
  ...
  if ( WIIC_USING_ACC(wm) ) {
    ...
    /* orientation */
    CROSS_THRESH( wm->lstate.orient.angle, wm->orient.angle,
                  wm->orient_threshold );
  }
  ...
  case EXP_NUNCHUK: {
    ...
    CROSS_THRESH( wm->lstate.exp_orient.angle, wm->exp.nunchuk.orient.angle,
  ...
  case EXP_MOTION_PLUS: {
    ...
    /* raw gyro rate */
    CROSS_THRESH_RATE( wm->lstate.mp_raw_gyro, wm->exp.mp.raw_gyro,
                       wm->exp.mp.raw_gyro_threshold );
    break;
  }
  case EXP_BALANCE_BOARD: {
    /* balance board */
    STATE_CHANGED( wm->lstate.pressure_raw_data.top_left,
                   wm->exp.bb.pressure_raw_data.top_left );
    STATE_CHANGED( wm->lstate.pressure_raw_data.top_right,
                   wm->exp.bb.pressure_raw_data.top_right );
    STATE_CHANGED( wm->lstate.pressure_raw_data.bottom_left,
                   wm->exp.bb.pressure_raw_data.bottom_left );
    STATE_CHANGED( wm->lstate.pressure_raw_data.bottom_right,
                   wm->exp.bb.pressure_raw_data.bottom_right );
    break;
  }
  case EXP_NONE: {
    break;
