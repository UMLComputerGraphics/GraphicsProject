...
void wiic_disconnected( struct wiimote_t* wm ) {
  ...
  // Auto-reconnect?
  if ( wm->autoreconnect && wiic_connect_single( wm, NULL, 1 ) ) return;
...
void wiic_rumble( struct wiimote_t* wm, int status ) {
  ...
  /* preserve IR state */
  if ( WIIMOTE_IS_SET(wm, WIIMOTE_STATE_IR) ) buf |= 0x04;
 ...
 *	finishes.
 */
int wiic_read_data_cb( struct wiimote_t* wm, wiic_read_cb read_cb, byte* buffer,
 ...
 *	finishes.
 */
int wiic_read_data( struct wiimote_t* wm, byte* buffer, unsigned int addr,
                    unsigned short len ) {
  struct read_req_t* req;
 ...
 */
struct wiimote_t* wiic_get_by_id( struct wiimote_t** wm, int wiimotes,
                                  int unid ) {
  int i = 0;
  
  for ( ; i < wiimotes; ++i ) {
    if ( wm[i]->unid == unid ) return wm[i];
  }
 ...
 */
int wiic_write_data( struct wiimote_t* wm, unsigned int addr, byte* data,
                     byte len ) {
...
int wiic_send( struct wiimote_t* wm, byte report_type, byte* msg, int len ) {
  ...
  case WM_CMD_REPORT_TYPE: {
    ...
    if ( WIIMOTE_IS_SET(wm, WIIMOTE_STATE_RUMBLE) ) rumble = 1;
    break;
  }
  default:
    break;
  ...
  else buf[2] &= 0xFE;	// Fix for Wiiuse bug
...
float wiic_set_smooth_alpha( struct wiimote_t* wm, float alpha ) {
  ...
  if ( wm->exp.type == EXP_NUNCHUK ) wm->exp.nunchuk.accel_calib.st_alpha =
      alpha;
