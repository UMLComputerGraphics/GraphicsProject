...
int wiic_find( struct wiimote_t** wm, int max_wiimotes, int timeout ) {
  ...
  if ( timeout ) found_devices = hci_inquiry( device_id, timeout, 128, NULL,
                                              &scan_info, IREQ_CACHE_FLUSH );
  else {
    while ( !(found_devices = hci_inquiry( device_id, 5, 128, NULL, &scan_info,
                                           IREQ_CACHE_FLUSH )) )
      ; // Unlimited inquiry
  }
 ...
 */
int wiic_connect_single( struct wiimote_t* wm, char* address,
                         int autoreconnect ) {
