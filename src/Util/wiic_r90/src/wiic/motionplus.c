 ...
 */
void calculate_gyro_rates( struct motion_plus_t* mp, struct ang3s_t* in,
                           struct ang3f_t* out ) {
  ...
  if ( mp->acc_mode & 0x04 ) tmp_roll = tmp_r / 20.0;
  else tmp_roll = tmp_r / 4.0;
  
  if ( mp->acc_mode & 0x02 ) tmp_pitch = tmp_p / 20.0;
  else tmp_pitch = tmp_p / 4.0;
  
  if ( mp->acc_mode & 0x01 ) tmp_yaw = tmp_y / 20.0;
  else tmp_yaw = tmp_y / 4.0;
...
void motion_plus_event( struct motion_plus_t* mp, byte* msg ) {
  ...
  mp->acc_mode = ((msg[4] & 0x2) << 1) | ((msg[3] & 0x1) << 1)
                 | ((msg[3] & 0x2) >> 1);
  ...
  // First calibration
  if ( (mp->a_raw_gyro.roll > 5000) && (mp->a_raw_gyro.pitch > 5000)
       && (mp->a_raw_gyro.yaw > 5000) && !(mp->cal_gyro.roll)
       && !(mp->cal_gyro.pitch) && !(mp->cal_gyro.yaw) ) {
    wiic_calibrate_motion_plus( mp );
  }
  
  if ( mp->smooth ) motion_plus_apply_smoothing( mp );
  else {
...
void motion_plus_apply_smoothing( struct motion_plus_t *mp ) {
  ...
  mp->raw_gyro.roll = alpha * mp->raw_gyro.roll
                      + (1.0 - alpha) * mp->a_raw_gyro.roll;
  mp->raw_gyro.pitch = alpha * mp->raw_gyro.pitch
                       + (1.0 - alpha) * mp->a_raw_gyro.pitch;
  mp->raw_gyro.yaw = alpha * mp->raw_gyro.yaw
                     + (1.0 - alpha) * mp->a_raw_gyro.yaw;
}
...
int motion_plus_handshake( struct wiimote_t* wm, byte* data,
  ...
  unsigned int val = (data[2] << 24) | (data[3] << 16) | (data[4] << 8)
                     | data[5];
...
void wiic_set_motion_plus( struct wiimote_t *wm, int status ) {
  ...
  if ( WIIMOTE_IS_SET(wm,WIIMOTE_STATE_EXP_HANDSHAKE) ) return;
  ...
  if ( status ) {
    ...
    wiic_read_data_cb( wm, motion_plus_handshake, tmp, WM_MOTION_PLUS_ID_ADDR,
                       6 );
  } else {
