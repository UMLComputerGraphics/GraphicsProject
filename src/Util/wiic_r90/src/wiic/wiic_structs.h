...
typedef struct wiimote_t {
  ...
  struct vec3b_t accel; /**< current raw acceleration data			*/
  WCONST
  struct orient_t orient; /**< current orientation on each axis (smoothed and unsmoothed)		*/
  WCONST
  struct gforce_t gforce; /**< current gravity forces on each axis (smoothed and unsmoothed)	*/
  WCONST
  float orient_threshold; /**< threshold for orient to generate an event */
  WCONST
  int accel_threshold; /**< threshold for accel to generate an event */
  ...
  WCONST
  WIIC_EVENT_TYPE event; /**< type of event that occured				*/
  WCONST
