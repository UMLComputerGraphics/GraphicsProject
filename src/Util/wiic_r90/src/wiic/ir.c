...
void wiic_set_ir_position( struct wiimote_t* wm, enum ir_position_t pos ) {
  ...
  case WIIC_IR_ABOVE:
    ...
    if ( wm->ir.aspect == WIIC_ASPECT_16_9 ) wm->ir.offset[1] = WM_ASPECT_16_9_Y
                                                                / 2
                                                                - 70;
  ...
  case WIIC_IR_BELOW:
    ...
    if ( wm->ir.aspect == WIIC_ASPECT_16_9 ) wm->ir.offset[1] =
        -WM_ASPECT_16_9_Y / 2 + 100;
    else if ( wm->ir.aspect == WIIC_ASPECT_4_3 )
...
void wiic_set_ir_sensitivity( struct wiimote_t* wm, int level ) {
  ...
  WIIMOTE_DISABLE_STATE(
      wm,
      (WIIMOTE_STATE_IR_SENS_LVL1 | WIIMOTE_STATE_IR_SENS_LVL2 | WIIMOTE_STATE_IR_SENS_LVL3 | WIIMOTE_STATE_IR_SENS_LVL4 | WIIMOTE_STATE_IR_SENS_LVL5) );
...
void calculate_basic_ir( struct wiimote_t* wm, byte* data ) {
  ...
  for ( i = 0; i < 4; ++i ) {
    if ( dot[i].ry == 1023 ) dot[i].visible = 0;
    else {
      dot[i].visible = 1;
...
static void interpret_ir_data( struct wiimote_t* wm ) {
  ...
  if ( WIIMOTE_IS_SET(wm, WIIMOTE_STATE_ACC) ) roll = wm->orient.angle.roll;
  
  /* count visible dots */
  wm->ir.num_dots = 0;
  for ( i = 0; i < 4; ++i ) {
    if ( dot[i].visible ) wm->ir.num_dots++;
  }
...
static void reorder_ir_dots( struct ir_dot_t* dot ) {
  ...
  for ( order = 1; order < 5; ++order ) {
    ...
    for ( j = 0; j < 4; ++j ) {
      if ( dot[j].visible && !dot[j].order && (dot[j].x < dot[i].x) ) i = j;
    }
