...
//	printf("\n\n--- EVENT [id %i] ---\n", wm->unid);
   ...
   */
  if ( IS_JUST_PRESSED(wm, WIIMOTE_BUTTON_MINUS) ) wiic_motion_sensing( wm, 0 );
  
  /*
   *	Pressing plus will tell the wiimote we are interested in movement.
   */
  if ( IS_JUST_PRESSED(wm, WIIMOTE_BUTTON_PLUS) ) wiic_motion_sensing( wm, 1 );
  
  if ( IS_JUST_PRESSED(wm, WIIMOTE_BUTTON_LEFT) ) wiic_set_motion_plus( wm, 0 );
   ...
   */
  if ( IS_JUST_PRESSED(wm, WIIMOTE_BUTTON_B) ) wiic_toggle_rumble( wm );
  
  if ( IS_JUST_PRESSED(wm, WIIMOTE_BUTTON_UP) ) wiic_set_ir( wm, 1 );
  if ( IS_JUST_PRESSED(wm, WIIMOTE_BUTTON_DOWN) ) wiic_set_ir( wm, 0 );
  ...
  if ( WIIC_USING_MOTION_PLUS(wm) ) {
    printf( "motion plus roll rate = %f [%f]\n", wm->exp.mp.gyro_rate.roll,
            wm->exp.mp.a_gyro_rate.roll );
    printf( "motion plus pitch rate = %f [%f]\n", wm->exp.mp.gyro_rate.pitch,
            wm->exp.mp.a_gyro_rate.pitch );
    printf( "motion plus yaw rate = %f [%f]\n", wm->exp.mp.gyro_rate.yaw,
            wm->exp.mp.a_gyro_rate.yaw );
  }
...
void handle_ctrl_status( struct wiimote_t* wm ) {
  ...
  printf( "ir:              %i\n", WIIC_USING_IR(wm) );
  printf( "leds:            %i %i %i %i\n", WIIC_IS_LED_SET(wm, 1),
          WIIC_IS_LED_SET(wm, 2), WIIC_IS_LED_SET(wm, 3),
          WIIC_IS_LED_SET(wm, 4) );
  printf( "battery:         %f %%\n", wm->battery_level );
}
