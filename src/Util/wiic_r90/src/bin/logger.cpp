...
void acquireGesture( CWii& wii, CWiimote& wiimote, const string& logfile,
                     bool resume ) {
  ...
  while ( enbAcq ) {
    ...
    // Activate logging
    if ( enbAcq == 1 && wiimote.Buttons.isJustPressed( CButtons::BUTTON_PLUS )
         && wiimote.Buttons.isPressed( CButtons::BUTTON_PLUS ) )
...
void randomAcquisition( CWii& wii, CWiimote& wiimote, const int samples,
                        vector< string >& files ) {
  // Total iterations
  ...
  for ( int i = 0; i < totSamples; i++ ) {
    ...
    if ( samples - fileSamples[selection] ) acquireGesture( wii, wiimote,
                                                            files[selection],
                                                            true );
    else acquireGesture( wii, wiimote, files[selection], false );
...
void timeAcquisition( CWii& wii, CWiimote& wiimote, const string logfile,
...
void dataAcquisition( CWii& wii, CWiimote& wiimote, const string& logfile,
  ...
  while ( enbAcq ) {
    ...
    if ( enbAcq == 1 && wiimote.Buttons.isJustPressed( CButtons::BUTTON_PLUS )
         && wiimote.Buttons.isPressed( CButtons::BUTTON_PLUS ) ) {
      if ( !count ) wiimote.LogStart( type, logfile );
      else wiimote.LogStart( type );
    }
    ...
    // End
    if ( enbAcq == 1
         && wiimote.Buttons.isJustPressed( CButtons::BUTTON_HOME ) ) {
      wiimote.LogStop();
      enbAcq = 0;
    }
  }
...
int main( int argc, char** argv ) {
  ...
  if ( argc == 2 ) {
    ...
    if ( option == "help" ) {
      cout << endl
           << "wiic-logger is a utility to collect accelerometer data in a log."
           << endl;
      cout << "wiic-logger is part of WiiC (http://wiic.sf.net)" << endl
           << endl;
      cout << "Usage: wiic-logger <filename> [mp]" << endl;
      cout
          << "       wiic-logger RANDOM <num_samples> <filename1> ... <filenameN>"
          << endl;
      cout << "       wiic-logger TIME <start_time> <length> <filename>"
           << endl;
      cout << "   <filename>: full pathname of the output log file" << endl;
      cout << "   <num_samples>: number of samples for each file" << endl
           << endl;
      cout
          << "   <filename1> ... <filenameN>: full pathname of each gesture file"
          << endl << endl;
      cout << "	<start_time>: sleep time (in seconds) before automatic logging"
           << endl;
    ...
    if ( option != "RANDOM" && option != "TIME" && mp != "mp" ) {
      ...
      cout << "Type wiic-logger help for a brief documentation." << endl
           << endl;
