...
void acc2robot( CWiimote &wm ) {
  ...
  if ( enable ) {
    ...
    robotJog = 0.0;
    else robotJog = MAX_JOG * roll * 2.0 / M_PI;
    ...
    robotSpeed = 0.0;
    else robotSpeed = MAX_SPEED * pitch * 2.0 / M_PI;
...
int main( int argc, char** argv ) {
  ...
  if ( argc == 2 ) {
    ...
    if ( option == "help" ) {
      cout
          << endl
          << "wiic-robot is a simple teleoperation interface to control a robot with a Wiimote."
          << endl;
