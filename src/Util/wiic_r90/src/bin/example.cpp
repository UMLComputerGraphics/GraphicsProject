...
void HandleEvent( CWiimote &wm ) {
  ...
  if ( exType == wm.ExpansionDevice.TYPE_NUNCHUK ) {
    ...
    printf( "%s L button pressed = %f\n", prefixString,
            cc.GetLShoulderButton() );
    printf( "%s R button pressed = %f\n", prefixString,
            cc.GetRShoulderButton() );
    ...
    bb.WeightSensor.GetWeight( total, topLeft, topRight, bottomLeft,
                               bottomRight );
...
void HandleStatus( CWiimote &wm ) {
  ...
  printf( "speaker: %i\n", wm.isUsingSpeaker() );
  printf( "ir: %i\n", wm.isUsingIR() );
  printf( "leds: %i %i %i %i\n", wm.isLEDSet( 1 ), wm.isLEDSet( 2 ),
          wm.isLEDSet( 3 ), wm.isLEDSet( 4 ) );
  printf( "battery: %f %%\n", wm.GetBatteryLevel() );
}
...
int main( int argc, char** argv ) {
  ...
  cout
      ...
      << endl;
  cout << "Press UP (DOWN) to enable (disable) IR camera (requires some IR led)"
       << endl;
