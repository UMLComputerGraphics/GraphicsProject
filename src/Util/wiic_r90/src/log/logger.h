...
namespace WiiC {
   ...
   */
  enum LogStatus {
    WIIC_LOG_START = 0x0, WIIC_LOG_STOP = 0x1
  };
}
...
public:
  Logger() :
      deviceAddr( "" ), logFile( "" ), relativeTs( 0.0 ), logEnabled( false ),
      logType( WIIC_LOG_NONE ), logStatus( WIIC_LOG_STOP ) {
  }
  Logger( const Logger& logger ) :
      deviceAddr( logger.deviceAddr ), logFile( logger.logFile ),
      startTs( logger.startTs ), relativeTs( logger.relativeTs ),
      logEnabled( logger.logEnabled ), logType( logger.logType ),
  ...
  void SetLogLevel( LogStatus status, int type = WIIC_LOG_NONE,
                    const string& file = "" );
  ...
  inline void SetLogStatus( const LogStatus status ) {
    logStatus = status;
  }
  ...
  inline void ResetTimestamp() {
    gettimeofday( &startTs, 0 );
  }
  inline void SetDeviceAddress( const string& addr ) {
    deviceAddr = addr;
  }
