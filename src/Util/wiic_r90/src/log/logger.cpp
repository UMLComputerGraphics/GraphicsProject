...
void Logger::SetLogLevel( LogStatus status, int type, const string& file ) {
  // Avoid repetitions
  if ( status == logStatus && type == logType && file == logFile ) return;
  
  if ( status == WIIC_LOG_START ) {
    ResetTimestamp();
    
    if ( (file == "" && logFile != "") || (file == logFile) ) { // Same file
      if ( logEnabled ) out << "END" << endl;
      
      // New training
      unsigned long trainingTs = (startTs.tv_sec % 86400) * 1000
                                 + startTs.tv_usec / 1000;
...
bool Logger::CheckLogFile() {
  ...
  if ( wiicToken == "WiiC" && logVersion <= WIIC_LOG_VERSION ) return true;
...
void Logger::InitLog() {
  ...
  struct timeval t;
  gettimeofday( &t, 0 );
  relativeTs = ((t.tv_sec * 1000000.0 + t.tv_usec)
                - (startTs.tv_sec * 1000000.0 + startTs.tv_usec))
               / 1000.0;
}
...
void Logger::LogGyro( float roll, float pitch, float yaw ) {
  out << "GYRO " << relativeTs << " " << roll << " " << pitch << " " << yaw
      << endl;
}
