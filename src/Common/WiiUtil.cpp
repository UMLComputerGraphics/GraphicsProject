...
bool initWii( CWii &wii ) {
  ...
  for ( size_t attempts = 0; attempts < 5; ++attempts ) {
    ...
    std::cerr
        << "Please place at least one Wii Remote and one Wii Balance Board in discoverable mode.\n";
    std::cerr
        << "The Wii Remote can be placed in discoverable mode by pressing 1+2 simultaneously.\n";
    std::cerr
        << "The Balance Board has a SYNC button under the battery cover on the bottom.\n";
    ...
    for ( index = 0, it = wiimotes->begin(); it != wiimotes->end();
        ++it, ++index ) {
      CWiimote &wm = *it;
...
void wiiHandleBB( CWiimote &wm ) {
  ...
  bb.WeightSensor.GetWeight( raw_val[TOT_WEIGHT], raw_val[TOP_LEFT],
                             raw_val[TOP_RIGHT], raw_val[BOT_LEFT],
                             raw_val[BOT_RIGHT] );
  ...
  float surge_pct = (adj_val[TOP_LEFT] + adj_val[TOP_RIGHT] - adj_val[BOT_LEFT]
                     - adj_val[BOT_RIGHT])
                    / adj_val[TOT_WEIGHT];
  float sway_pct = (adj_val[TOP_RIGHT] + adj_val[BOT_RIGHT] - adj_val[TOP_LEFT]
                    - adj_val[BOT_LEFT])
                   / adj_val[TOT_WEIGHT];
...
void pollWii( CWii &wii, bool CalibrateGyro ) {
  ...
  if ( 0 ) {
    ...
    size_t i, j;
    if ( (i = end.tv_sec - start.tv_sec) == 0 ) j = end.tv_usec - start.tv_usec;
